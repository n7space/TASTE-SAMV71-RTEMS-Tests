...
def test_samv71_rtems_boot_reason():
    ...
    build = common.do_build(
        "samv71-rtems-boot-reason/TEST-SAMV71-FAULT", ["samv71", "debug"]
    )
    ...
    try:
        ...
        gdbmi.write(
            "file samv71-rtems-boot-reason/TEST-SAMV71-FAULT/work/binaries/partition_1"
        )
    ...
    build = common.do_build(
        "samv71-rtems-boot-reason/TEST-SAMV71-BOOT-REASON", ["samv71", "debug"]
    )
    stderr = build.stderr.decode("utf-8")
    assert build.returncode == 0, f"Compilation errors: \n{stderr}"
    ...
    common.run_verification_project(
        remote_gdb_server,
        "samv71-rtems-boot-reason/TEST-SAMV71-BOOT-REASON/work/binaries/partition_1",
        "function_1.c",
        "22",
    )
