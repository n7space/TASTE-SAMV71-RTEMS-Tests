...
def test_samv71_rtems_twin_serial():
    remote_gdb_server = os.getenv("SAMV71_REMOTE_GDBSERVER", default="127.0.0.1")
    build = common.do_build("samv71-rtems-twin-serial", ["deploymentview", "debug"])
    ...
    try:
        ...
        gdbmi.write("file samv71-rtems-twin-serial/work/binaries/partition_2")
        ...
        errors = common.do_execute(
            "samv71-rtems-twin-serial", expected, test_exe="work/binaries/partition_1"
        )
    finally:
        gdbmi.exit()
    assert not errors, "\n".join(errors)
